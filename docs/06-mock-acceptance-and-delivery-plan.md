...
| 用户流程 | Mock 行为 | 关键验收 |
| --- | --- | --- |
| 欢迎、绑定与账户 | 返回 Pro 用户组、额度、已用额度、剩余额度与请求数 | 欢迎页区分未绑定/已绑定；账户显示与网页币种一致；不展示密钥 |
...
1. Bot 模型列表只展示当前账户可用模型和端点类型。
2. 模型详情展示可解释的费率摘要，并跳转 `https://supertoken.cc/pricing/<model>` 查看完整规则。
3. 估算器必须收集模型、端点、输入/输出 token 或媒体参数，并明确显示“估算，不是结算承诺”。
4. 唯一的最终结算来源是 `new-api` 的实际扣费逻辑和用量日志。

`new-api` 新增的 `POST /api/integrations/telegram/v1/models` 只按已验证 Telegram 身份解析用户，并使用该用户分组已启用的模型；分页最多 12 项，避免将管理后台的全局模型表暴露给 Bot。

## 4. 真实集成前硬门槛

以下事项全部完成前，`TELEGRAM_TOPUP_ENABLED` 必须保持关闭，且不在生产菜单标记链上充值为可用：
...
| Telegram 私聊验收 | 已登录的 `@supertoken_bot` 测试会话；只读/Mock 操作 | 已见到账户 $250.00 / $87.50 / $162.50、24h/7 天/30 天用量、active/expired 订阅、63 项模型、公告、通知设置、中文/英文切换；微信 Mock 与 Base USDT Mock 均完成“建单 -> 确认中 -> 到账”显示 | `d7a7dfa04b6242b70a215555e9f9e8a6e7e553f5` |
